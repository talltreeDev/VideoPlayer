...
export function ProgressBar() {
  ...
  const skipTime = usePlayerStore((s) => s.interface.skipTimeList);
  const showAutoSkip = usePlayerStore((s) => s.interface.skipTimeShow);
  const display = usePlayerStore((s) => s.display);
  const setDraggingTime = usePlayerStore((s) => s.setDraggingTime);
  const setSeeking = usePlayerStore((s) => s.setSeeking);
  const { isSeeking, skipSettingShow } = usePlayerStore((s) => s.interface);
  const skipTimeCtr = usePlayerStore((s) => s.setAutoSkipTime);
  const showSkipSetting = usePlayerStore((s) => s.setAutoSkipValue);
  const [prevSkipValues, setPrevSkipValues] = useState<number[]>([]);
  const [skipValues, setSkipValues] = useState<number[]>([0, 0]);
  ...
  const handleSkipTimeCtr = () => {
    skipTimeCtr(skipValues);
    display?.setTime(0);
    showSkipSetting(false);
  };

  const handlePreviewAutoSkip = (values: number[]) => {
    setSkipValues(values);
    if (!prevSkipValues.length) {
      setPrevSkipValues(values);
    } else {
      if (prevSkipValues[0] !== values[0]) {
        display?.setTime(values[0]);
      } else if (prevSkipValues[1] !== values[1]) {
        display?.setTime(values[1]);
      }
      setPrevSkipValues(values);
    }
  };
  ...
  useEffect(() => {
    if (skipTime.length) {
      setStartTime(skipTime[0]);
      setEndTime(skipTime[1]);
    }
  }, [skipTime]);
        ...
        <ChakraProvider>
          <div className="flex justify-between items-center">
            <RangeSlider
              ...
              max={duration}
              step={1}
              onChangeEnd={(v: number[]) => handlePreviewAutoSkip(v)}
            >
              <RangeSliderTrack>
                <RangeSliderFilledTrack />
              </RangeSliderTrack>
            ...
            <div className="px-2">
              <Button onClick={handleSkipTimeCtr}>Add</Button>
            </div>
          </div>
        </ChakraProvider>
      )}
      ...
      <div className="w-full" ref={ref}>
        ...
        >
          ...
          >
            ...
            {showAutoSkip && (
              <div
                className="absolute top-0 dir-neutral:left-0 h-full rounded-full bg-yellow-300 flex justify-end items-center"
                style={{
                  width: `${
                    Math.max(0, (endTime - startTime) / duration) * 100
                  }%`,
